...
sed -i '/uci commit system/i\uci set system.@system[0].hostname='Redmi-AX6'' package/lean/default-settings/files/zzz-default-settings

# 版本号里显示一个自己的名字（ababwnq build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i 's/OpenWrt /编译时间 $(TZ=UTC-8 date "+%Y年%m月%d日") @ 情人小哥哥 /g' package/lean/default-settings/files/zzz-default-settings
...
rm -rf luci/themes/luci-theme-material
rm -rf luci/themes/luci-theme-netgear
...
#使用sed 在第四行后添加新字
sed -i '/set wireless.default_radio${devidx}.encryption=psk2/a\set wireless.default_radio${devidx}.key=password' package/kernel/mac80211/files/lib/wifi/mac80211.s
